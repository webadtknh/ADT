...
class Cdrr_Item extends Doctrine_Record {
	...
	public function getPositiveAdjustment($period_begin, $drug_id, $facility_id) {
		$query = Doctrine_Query::create() -> select("positive_adjustment") -> from("cdrr_item ci") -> where("ci.Cdrr.period_begin = '$period_begin' and ci.drug_id='$drug_id' and ci.Cdrr.facility_id='$facility_id' and ci.Cdrr.status !='prepared' and ci.Cdrr.status !='deleted'") -> OrderBy("ci.Cdrr.id desc");
		$items = $query -> execute();
		print(mysql_error());
		return @$items[0]['positive_adjustment'];
		//return $items['positive_adjustment'];
	}
